import React from 'react'
import './home.css'

export const Footer = () => {

    const baseUrl = "http://localhost:3002/";
    const footer = `${baseUrl}footer`;
    const [data, setData] = React.useState([])

    React.useEffect(() => {
        getFooter()
    },[])

    const getFooter = () => {
        fetch(footer)
            .then((res) => (res = res.json()))
            .then((res) => { setData(res) })
        console.log(data)
    }

    return (
        <div className='footer-container'>

            <div className='contact-div'>
                <div className='contact-wrapper'>

                    <div className='email'>
                        <div className='icon-div'>
                            <span></span>
                            <p className='footer-text bold'>Get special discount on your inbox</p>
                        </div>
                        <div className='email-send' style={{display: "flex"}}>
                            <input type="text" placeholder='Your Email' style={{borderbelow:"1px solid black"}}/>
                            <div style={{padding:"2%", border: "1px solid grey"}}><span className='footer-text bold send' style={{fontSize:"19px"}} >SEND</span></div>
                        </div>
                    </div>

                    <div className='app'>
                        <div className='icon-div'>
                            <span></span>
                            <p className='footer-text bold'>EXPERIENCE THE NYKAA MOBILE APP</p>
                        </div>
                        <div className='appstore' style={{display: "flex"}}>
                            <div><img className='app-img' src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRtW2CxVO2RXB--dHwFVDpn7kKWu5tQ90b-Qi6YKQ-1sI90eYvMaEWZytRLX-1aymKRNg&usqp=CAU" alt="google play" style={{width:"90%"}}/></div>
                            <div><img className='app-img' src="data:image/png;base64," alt="apptore" style={{width: "90%"}} /></div>
                        </div>
                    </div>

                    <div className='phone'>
                        <div className='icon-div'>
                            <span></span>
                            <p className='footer-text bold'>FOR ANY HELP YOU MAY CALL US AT 1800-267-4444</p>
                        </div>
                        <div className='phone-text-div' >
                            <div>
                                <p className='footer-text  bold phone-text'>(Monday to Saturday, 8AM to 10PM and Sunday, 10AM to 7PM)</p>
                            </div>
                        </div>
                    </div>

                </div>
            </div>


            <div className='footer-categories'>
                <div className='categories-wrapper'>
                    <div className='category-nykaa'>
                        <div className='nykaa-wrapper'>
                            <div><span>Nykaa</span></div>
                            {
                                data.map((item) => {
                                    if (item.name === "nyka") {
                                        return <p className='footer-text'>{item.text}</p>
                                    }
                                })
                            }
                        </div>
                    </div>

                    <div className='category-help'>
                        <div className='help-wrapper'>
                            <div><span>Help</span></div>
                            {
                                data.map((item) => {
                                    if (item.name === "help") {
                                        return <p className='footer-text'>{item.text}</p>
                                    }
                                })
                            }
                        </div>
                    </div>

                    <div className='category-inspireMe'>
                        <div className='inspire-wrapper'>
                            <div><span>Help</span></div>
                            {
                                data.map((item) => {
                                    if (item.name === "inspireMe") {
                                        return <p className='footer-text'>{item.text}</p>
                                    }
                                })
                            }
                        </div>
                    </div>

                    <div className='category-quicklinks'>
                        <div className='quicklinks-wrapper'>
                            <div><span>Quick Links</span></div>
                            {
                                data.map((item) => {
                                    if (item.name === "quicklinks") {
                                        return <p className='footer-text'>{item.text}</p>
                                    }
                                })
                            }
                        </div>
                    </div>

                    <div className='category-topcategories'>
                        <div className='topcategories-wrapper'>
                            <div><span>Top Categories</span></div>
                            {
                                data.map((item) => {
                                    if (item.name === "topcategories") {
                                        return <p className='footer-text'>{item.text}</p>
                                    }
                                })
                            }
                        </div>
                    </div>
                </div>
            </div>

            <div className='services-footer'>
                <div className='services-wrapper'>
                    <div className='service-shipping service'>
                        <div >
                            <img className='service-icon' src="https://adn-static2.nykaa.com/media/wysiwyg/2021/Authenticity.svg" alt="" />

                        </div>
                        <div>
                            <p className='heading-service'>FREE SHIPPING</p>
                            <hr />
                            <p className='desc-service'>for orders above INR 499</p>
                        </div>
                    </div>

                    <div className='service-returns service'>
                        <div >
                            <img className='service-icon' src="https://adn-static2.nykaa.com/media/wysiwyg/2021/return_accepted.svg" alt="" />
                        </div>
                        <div>
                            <p className='heading-service'>EASY RETURNS</p>
                            <hr />
                            <p className='desc-service'>15 Day Easy Returns For Most Products</p>
                        </div>
                    </div>

                    <div className='service-authentic service'>
                        <div >
                            <img className='service-icon' src="https://adn-static2.nykaa.com/media/wysiwyg/2021/Authenticity.svg" alt="" />
                        </div>
                        <div>
                            <p className='heading-service'>AUTHENTIC PRODUCTS</p>
                            <hr />
                            <p className='desc-service'>Sourced Directly from Brands/ Authorised Distributors</p>
                        </div>
                    </div>

                    <div className='service-brands service'>
                        <div >
                            <img className='service-icon' src="https://adn-static2.nykaa.com/media/wysiwyg/2021/Brands.svg" alt="" />
                        </div>
                        <div>
                            <p className='heading-service'>2400+ BRANDS</p>
                            <hr />
                            <p className='desc-service'>Well Curated 1.9Lakh + Product</p>
                        </div>
                    </div>

                    <div className='service-brands service'>

                        <div>
                            <p >show us some love ❤ on social media</p>
                            <div>
                                <a href="https://www.instagram.com/mynykaa/?ref=badge">
                                    <img className='social-icon' src="	https://images-static.naikaa.com/media/wysiwyg/2021/icons/ic_social-instagram-filled.svg" alt="" />
                                </a>
                                <a href="https://www.facebook.com/MyNykaa">
                                    <img className='social-icon' src="	https://images-static.naikaa.com/media/wysiwyg/2021/icons/ic_social-facebook-filled.svg" alt="" />
                                </a>
                                <a href="https://www.youtube.com/channel/UCoaH2UtB1PsV7av17woV1BA">
                                    <img className='social-icon' src="	https://images-static.naikaa.com/media/wysiwyg/2021/icons/ic_social-youtube-filled.svg" alt="" />
                                </a>
                                <a href="https://twitter.com/MyNykaa">
                                    <img className='social-icon' src="	https://images-static.naikaa.com/media/wysiwyg/2021/icons/ic_social-twitter-filled.svg" alt="" />
                                </a>
                                <a href="https://www.pinterest.com/mynykaa/">
                                    <img className='social-icon' src="	https://images-static.naikaa.com/media/wysiwyg/2021/icons/ic_social-pinterest-filled.svg" alt="" />
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div className='footer-terms'>
                <div className='terms-wrapper'>
                    <div className='terms1'>
                        <div className='terms1-wrapper'>
                            <p className='terms-text'>Terms & Conditions</p>
                            <p className='terms-text'>Shipping Policy</p>
                            <p className='terms-text'>Cancellation Policy</p>
                            <p className='terms-text'>Privacy Policy</p>
                        </div>
                    </div>
                    <p className='trc'>
                        <span>©</span>
                        2022
                        Nykaa E-Retail Pvt. Ltd. All Rights Reserved
                    </p>
                </div>
            </div>

        </div>
    )
}


